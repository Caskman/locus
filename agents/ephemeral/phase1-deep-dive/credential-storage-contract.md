...
> **Note:** The `.commit()` method is blocking. These functions are marked `suspend` and explicitly use `Dispatchers.IO` to ensure Main safety while maintaining atomicity.

```kotlin
suspend fun promoteToRuntime(runtimeCreds: RuntimeCredentials) = withContext(Dispatchers.IO) {
...
```kotlin
suspend fun replaceWithAdmin(adminCreds: RuntimeCredentials) = withContext(Dispatchers.IO) {
    sharedPreferences.edit()
