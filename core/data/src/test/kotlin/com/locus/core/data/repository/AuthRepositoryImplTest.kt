...
class AuthRepositoryImplTest {
    ...
    private val bootstrapCreds = BootstrapCredentials(
        accessKeyId = "ASIA...",
    ...
    private val runtimeCreds = RuntimeCredentials(
        accessKeyId = "AKIA...",
        secretAccessKey = "secret",
    ...
    @Test
    fun `init loads Authenticated state when Runtime credentials exist`() = runTest {
    ...
    @Test
    fun `init loads SetupPending state when Bootstrap credentials exist`() = runTest {
        coEvery { secureStorage.getRuntimeCredentials() } returns LocusResult.Success(null)
    ...
    @Test
    fun `init loads Uninitialized state when no credentials exist`() = runTest {
        coEvery { secureStorage.getRuntimeCredentials() } returns LocusResult.Success(null)
        coEvery { secureStorage.getBootstrapCredentials() } returns LocusResult.Success(null)
    ...
    @Test
    fun `promoteToRuntimeCredentials saves runtime, deletes bootstrap, and updates state`() = runTest {
    ...
    @Test
    fun `validateBucket returns Available when bucket exists and has correct tag`() = runTest {
        coEvery { secureStorage.getBootstrapCredentials() } returns LocusResult.Success(bootstrapCreds)
        ...
        coEvery { s3Client.getBucketTagging(any<GetBucketTaggingRequest>()) } returns GetBucketTaggingResponse {
            tagSet = listOf(Tag { key = "LocusRole"; value = "DeviceBucket" })
        }
    ...
    @Test
    fun `validateBucket returns Invalid when tag is missing`() = runTest {
        coEvery { secureStorage.getBootstrapCredentials() } returns LocusResult.Success(bootstrapCreds)
        ...
        coEvery { s3Client.getBucketTagging(any<GetBucketTaggingRequest>()) } returns GetBucketTaggingResponse {
            tagSet = listOf(Tag { key = "OtherRole"; value = "SomethingElse" })
        }
    ...
    @Test
    fun `scanForRecoveryBuckets filters correctly`() = runTest {
        coEvery { secureStorage.getBootstrapCredentials() } returns LocusResult.Success(bootstrapCreds)

        val s3Client = mockk<S3Client>(relaxed = true)
        every { awsClientFactory.createBootstrapS3Client(any()) } returns s3Client

        coEvery { s3Client.listBuckets() } returns ListBucketsResponse {
            buckets = listOf(
    ...
    @Test
    fun `recoverAccount parses stack outputs correctly`() = runTest {
        coEvery { secureStorage.getBootstrapCredentials() } returns LocusResult.Success(bootstrapCreds)

        val s3Client = mockk<S3Client>(relaxed = true)
        every { awsClientFactory.createBootstrapS3Client(any()) } returns s3Client
        coEvery { s3Client.getBucketTagging(any<GetBucketTaggingRequest>()) } returns GetBucketTaggingResponse {
            tagSet = listOf(Tag { key = "aws:cloudformation:stack-name"; value = "locus-stack" })
        }
        ...
        coEvery { cfClient.describeStacks(any<DescribeStacksRequest>()) } returns DescribeStacksResponse {
            stacks = listOf(Stack {
                outputs = listOf(
                    Output { outputKey = "RuntimeAccessKeyId"; outputValue = "AKIA_RUNTIME" },
                    Output { outputKey = "RuntimeSecretAccessKey"; outputValue = "SECRET_RUNTIME" },
                    Output { outputKey = "BucketName"; outputValue = "locus-bucket-recovered" }
                )
            })
        }
