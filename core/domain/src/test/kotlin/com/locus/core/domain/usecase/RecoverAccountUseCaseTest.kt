...
class RecoverAccountUseCaseTest {
    ...
    fun `returns failure when bucket tags fetch fails`() =
        runBlocking {
            ...
            coVerify { authRepository.updateProvisioningState(match { it is ProvisioningState.Working && it.currentStep == "Validating bucket metadata..." }) }
    ...
    fun `returns failure when stack creation fails`() =
        runBlocking {
            ...
            coVerify { authRepository.updateProvisioningState(match { it is ProvisioningState.Working && it.currentStep == "Initiating recovery stack deployment..." }) }
        }
